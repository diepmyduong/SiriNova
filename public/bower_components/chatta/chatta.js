...
window.chatta = (function(config) {
  ...
  function MessageElement(message) {
    ...
    var $chattaMssgRow     = new Element('div', 'chatta_mssg_row_'+_id);
    var $chattaMssgWrap    = new Element('div', 'chatta_mssg_'+_id);
    var $chattaMssgAvatar  = new Element('div', 'chatta_mssg_avatar_'+_id);
    var $chattaMssgAvatarImg   = new Element('img', 'chatta_mssg_avatar_img_'+_id);
    var $chattaMssgContent = new Element('div', 'chatta_mssg_content_'+_id);
    var $chattaMssgMeta    = new Element('div', 'chatta_mssg_meta_'+_id);
    

    // Chat message wrapper styles
    $chattaMssgWrap.addStyles({
    ...
    $chattaMssgContent.addStyles({
      'padding':'7px 10px'
    });

    // Chat message meta styles
    $chattaMssgMeta.addStyles({
    ...
    $chattaMssgAvatarImg.addStyles({
      'height': '40px',
      'width':'40px'
    });
    $chattaMssgAvatarImg.attr('sr-src',message.avatarSrc);
    $chattaMssgAvatarImg.attr('class','img-circle');

    // $chattaMssgAvatar
    $chattaMssgContent.setText(message.content);
    $chattaMssgMeta.setText('Gửi bởi ' + message.from + ' at ' + message.date.format());
    $chattaMssgWrap
      .append($chattaMssgContent)
      .append($chattaMssgMeta);
    $chattaMssgAvatar
      .append($chattaMssgAvatarImg);
    $chattaMssgRow
      .append($chattaMssgAvatar)
      .append($chattaMssgWrap);
    return $chattaMssgRow;
  }
  ...
  var $chatta = {_isUp: false};
  $chatta.init = function(callback) {
    console.log("init chatta");
    // --------------------------------------------------
    // -- Build the widget
    // --------------------------------------------------

    // Create elements
    $chatta.$chattaWrap       = new Element('div', 'chatta_wrap_'+parseInt(1000*Math.random()));
    $chatta.$chattaTab        = new Element('div', 'chatta_tab');
    $chatta.$chattaTabText    = new Element('span', 'chatta_tab_text');
    $chatta.$chattaTabClose   = new Element('span', 'chatta_tab_close');
    $chatta.$chattaTabArrow   = new Element('span', 'chatta_tab_arrow');
    $chatta.$chattaBox        = new Element('div', 'chatta_box');
    $chatta.$chattaFormWrap   = new Element('div', 'chatta_form_wrap');
    $chatta.$chattaForm       = new Element('form', 'chatta_form');
    $chatta.$chattaEmailField = new Element('input', 'chatta_email_field');
    $chatta.$chattaNameField  = new Element('input', 'chatta_name_field');
    $chatta.$chattaMessageBox = new Element('textarea', 'chatta_message_box');
    $chatta.$chattaSubmit     = new Element('input', 'chatta_submit');
    $chatta.$chattaUserDetails= new Element('div', 'chatta_user_details');
    $chatta.$chattaLight      = new Element('span', 'chatta_light');

    // Set attributes if needed
    $chatta.$chattaTabArrow.setText(upArrow);
    $chatta.$chattaTabClose.attr('class','glyph-icon icon-remove');
    $chatta.$chattaTabText.setText(config&&config.label||'Chat to the team!');
    $chatta.$chattaEmailField.attr('type', 'text');
    $chatta.$chattaEmailField.attr('placeholder', 'Your email');
    $chatta.$chattaNameField.attr('type', 'text');
    $chatta.$chattaNameField.attr('placeholder', 'Your name');
    $chatta.$chattaMessageBox.attr('placeholder', 'Your message');
    $chatta.$chattaSubmit.attr('type', 'submit');
    $chatta.$chattaSubmit.attr('value', 'Send');

    // Add styles
    $chatta.$chattaWrap.addStyles({
      'width':widgetWidth + 'px',
      'background':'#fff',
      'position':'fixed',
      'bottom':'-500px',
    ...
    $chatta.$chattaTab.addStyles({
      'padding':'10px 20px',
      'background':config.tabColor||'#cd5a54',
      'color':'#fff'
    });

    $chatta.$chattaLight.addStyles({
      'background':config.chattaActiveColor||'#36c498',
      'margin-right':'10px',
      'width':'10px',
      'height':'10px',
    ...
    $chatta.$chattaTabArrow.addStyles({
      'color':'#fff',
      'float':'right'
    });
    $chatta.$chattaTabClose.addStyles({
      'color':'#fff',
      'float':'right',
      'margin-right': '10px'
    });

    $chatta.$chattaBox.addStyles({
      'max-height':'150px',
      'overflow':'scroll',
      'font-size':'11px',
      'background':'#eee'
    ...
    $chatta.$chattaFormWrap.addStyles({
      'width': widgetWidth + 'px',
      'padding':'10px',
      'bottom':'0px',
      'background':'#fff',
    ...
    $chatta.$chattaNameField.addStyles(chattaInputStyle);
    $chatta.$chattaEmailField.addStyles(chattaInputStyle);
    $chatta.$chattaMessageBox.addStyles(chattaInputStyle);

    $chatta.$chattaSubmit.addStyles({
      'width':(widgetWidth-20)+'px',
      'border':'none',
      '-webkit-border-radius':'3px',
      '-webkit-border-radius':'3px',
    ...
    $chatta.$chattaUserDetails.addStyles({
      'text-align':'center',
      'background':config.userDetailsFooterColor||'#36c498',
      'color':'#fff',
      'padding':'4px',
    ...
    // Create tree
    $chatta.$chattaTab.append($chatta.$chattaLight);
    $chatta.$chattaTab.append($chatta.$chattaTabText);
    $chatta.$chattaTab.append($chatta.$chattaTabArrow);
    $chatta.$chattaTab.append($chatta.$chattaTabClose);
    $chatta.$chattaWrap.append($chatta.$chattaTab);
    $chatta.$chattaWrap.append($chatta.$chattaBox);
    $chatta.$chattaForm.append($chatta.$chattaNameField);
    $chatta.$chattaForm.append($chatta.$chattaEmailField);
    $chatta.$chattaForm.append($chatta.$chattaMessageBox);
    $chatta.$chattaForm.append($chatta.$chattaSubmit);
    $chatta.$chattaFormWrap.append($chatta.$chattaForm);
    $chatta.$chattaWrap.append($chatta.$chattaFormWrap);
    $chatta.$chattaWrap.append($chatta.$chattaUserDetails);


    var _this = this;
    _this._e = $chatta.$chattaWrap;
    // Render the widget on DOM
    // angular.element(document.body).append(angular.element(_this._e.getElement()));
    ...
    $chatta.$chattaTab.on('click', function() {
      _this.toggleBox();
    });
    ...
    $chatta.$chattaForm.on('submit', formSubmit);
    $chatta.$chattaEmailField.on('keyup', this._events.emailKeyup);
    $chatta.$chattaNameField.on('keyup', this._events.nameKeyup);
    $chatta.$chattaMessageBox.on('keydown', function(e) {
      e = e || event;
    ...
    $chatta.$chattaMessageBox.on('keyup', function(e) {
      e = e || event;
      $chatta._events.mssgKeyup(e);
      return true;
    });
  ...
  $chatta._getFormData = function() {
    ...
    data.message = $chatta.$chattaMessageBox.val();
    if(data.message){
      data.message = data.message.replace(/(?:\r\n|\r|\n)/g, '');
    }
    data.user = {
      name: $chatta.$chattaNameField.val(),
      email: $chatta.$chattaEmailField.isValidEmail()&&$chatta.$chattaEmailField.val()
    };
    return data;
  };
  ...
  $chatta.setUser = function(user) {
    ...
    if(this._user.name && this._user.email){
      ...
      $chatta.$chattaUserDetails.setText('Messaging as ' + user.name + ' : ' + user.email);
      $chatta.$chattaUserDetails.addStyles({
        'display':'block'
      });
      return true;
  ...
  $chatta.toggleBox = function(callback) {
    ...
    if(_this._isUp) {
      ...
      _this._e.animateY(0, function() {
        _this._isUp = true;
        $chatta.$chattaTabArrow.setText(downArrow);
        (callback||noop)();
      });
    }
    return _this;
  ...
  $chatta.resetErrorState = function() {
    ...
    $chatta.$chattaEmailField.addStyles({
      'border':'1px solid #eee'
    });

    $chatta.$chattaNameField.addStyles({
      'border':'1px solid #eee'
    });
  ...
  $chatta.setErrors = function(message, nameErr, emailErr) {
    ...
    // Set message text
    $chatta.$chattaUserDetails.setText(message || 'Form error');
  ...
  $chatta.goLeft = function() {
    var right = parseInt($chatta._e.css('right').replace("px", ""));
    var width = parseInt($chatta._e.css('width').replace("px", ""));
    document.getElementById($chatta._e._id).style.right = width+right+pacing+'px';
  }
  ...
  $chatta.scrollToBottom = function() {
    $chatta.$chattaBox.getElement().scrollTop = $chatta.$chattaBox.getElement().scrollHeight;
  };
  ...
  $chatta.setFormError = function(on, message) {
    if(on) {
      ...
      $chatta.$chattaSubmit.attr('value', message||'Error!');
      $chatta.$chattaSubmit.addStyles({
        'background':config.errorColor||'#cd5a54',
        'opacity':'0.7'
      });
      $chatta.$chattaSubmit.getElement().disabled = true;
    } else {
      $chatta.$chattaSubmit.attr('value', 'Send');
      ...
      $chatta.$chattaSubmit.getElement().disabled = false;
    }
  };
  ...
  $chatta.addMessage = function(message) {
    ...
    $chatta.$chattaBox.append($mssg);
    // var avatarEl = angular.element($($mssg.getElement()).find('[sr-src]'));
    // message.compile(avatarEl)(avatarEl.scope);
    // var $scope = angular.element($('#chatta_mssg_avatar_'+message.id)).scope();
    message.compile('#chatta_mssg_avatar_img_'+message.id)(message.scope);
    // message.scope.$appy();
    // $scope.addMessageAvatar('#chatta_mssg_avatar_img_'+message.id);
